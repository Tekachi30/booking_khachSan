...
const getImgRoom = async (req, res) => {
    try {
        const img = await ImgRoom.findAll();
    ...
    try {
        upload.single("avatar")(req, res, async function (err) {
            ...
            if (req.file) {
                ...
                await ImgRoom.create({url: imageUrl, name_img: req.file.filename})
...
const updateImgRoom = async (req, res) => {
    try {
        upload.single("avatar")(req, res, async function (err) {
            ...
            const exsitImgReport = await ImgRoom.findByPk(id);
            // Kiểm tra nếu có file ảnh mới được chọn
            if (req.file) {
                const imageUrl = `${req.protocol}://${req.get("host")}/${req.file.filename}`;
