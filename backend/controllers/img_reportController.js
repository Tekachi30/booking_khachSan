...
const getImgReport = async (req, res) => {
    try {
        const img = await ImgReport.findAll();
...
const addImgReport = async (req, res) => {
    try {
        upload.single("avatar")(req, res, async function (err) {
            ...
            if (req.file) {
                ...
                await ImgReport.create({url: imageUrl, name_img: req.file.filename})
...
const updateImgReport = async (req, res) => {
    try {
        upload.single("avatar")(req, res, async function (err) {
            ...
            const exsitImgReport = await ImgReport.findByPk(id);
            // Kiểm tra nếu có file ảnh mới được chọn
            if (req.file) {
                const imageUrl = `${req.protocol}://${req.get("host")}/${req.file.filename}`;
...
const deleteImgReport = async (req, res) => {
    try {
        const id = req.params.id;
        const exsitImgReport = await ImgReport.findByPk(id);
