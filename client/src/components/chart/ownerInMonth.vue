...
export default {
    ...
    mounted() {
        ...
        this.chart = new Chart(chartCanvas, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: `Số lượng chủ khách sạn được tạo theo từng tháng`,
                        data: [],
    ...
    methods: {
        async fetchData() {
            try {
                const response = await this.$axios.get(`dashboard/countOwnerInMonth?year=${this.year}`);
                const data = response.data.monthlyOwnerCount;
                this.showChart(data);
            } catch (error) {
                console.error(error);
            }
