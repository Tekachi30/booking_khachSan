...
export default {
    ...
    mounted() {
        ...
        this.chart = new Chart(chartCanvas, {
            ...
            data: {
                ...
                datasets: [
                    {
                        label: `Số lượng mã giảm giá được tạo theo từng tháng của khách sạn`,
                        data: [],
    ...
    methods: {
        ...
        async fetchData() {
            try {
                const response = await this.$axios.get(`dashboard/couponHotelInMonth?year=${this.year}&id=${this.hotel.id}`);
                const data = response.data.monthlyCoupon;
                this.showChart(data);
            } catch (error) {
                console.error(error);
            }
