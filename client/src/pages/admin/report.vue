                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Nội dung báo cáo</th>
                            ...
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row"
                                    class="px-4 py-3 font-medium  text-gray-900 whitespace-nowrap dark:text-black">{{ index +
                                        1 }}</th>
                                <td class="px-4 py-3">
                                    {{ report.hotel.name_hotel }}
                                </td>
                                <td class="px-4 py-3"> {{ report.User.fullname }}</td>
                                <td class="px-4 py-3"> {{ report.comment_report }}</td>
...
export default
{
  ...
  methods: {
    ...
    async getReport() {
       try {
           const result = await this.$axios.get('report/get');
    ...
    {
       try {
            const result = await this.$axios.post('report/search',
            {
